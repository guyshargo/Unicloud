...
**📊 UniCloud – Smart Task & Sensor Dashboard**

**Overview**

This project is a dynamic and interactive dashboard that combines task management and real-time IoT sensor monitoring, tailored for both employees and administrators. Built using **Python**, **IPyWidgets**, and **Firebase**, the system offers personalized user experiences, gamified tracking, and administrative control in a unified interface.
...
- View and manage personal tasks with status indicators.  
- Track progress and earn points based on task completion.  
- Access purchase history and gamified performance summaries.  

**📟 Sensor Dashboard:**

- Live monitoring of indoor/outdoor sensors (e.g., temperature, humidity, light).  
- Detection of abnormal values and automatic classification.  
- Real-time graphs, statistical summaries, and user-specific visibility.  

**🔍 Smart Search Engine:**

- Full-text search with indexed results.  
- Intelligent suggestions while typing.  
- Ranking algorithm based on keyword relevance and frequency.  

**🛠️ Admin Panel:**

- Separate tab for task and user management.  
- Toggle task visibility, assign roles, and monitor system usage.  
- View aggregated performance analytics across users.  
...
- **IPyWidgets** – UI components within Jupyter Notebook  
...
---
Page Rank Explanation:
- In the Search tab, each result includes a "Page Rank" value.
- Page Rank is a numeric score that reflects how relevant a result is to the user's query.
- It is calculated based on the number of times the search keywords appear in the page content.
- Higher Page Rank indicates a stronger match to the query.
---
Task Manager Tab:
- Admins can:
  - Add new tasks.
...
with tab8_content:
    ...
    password_input_text = widgets.Password(
